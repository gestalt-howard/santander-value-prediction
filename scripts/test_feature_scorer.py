...
def get_scores(train, f, lagval):
    ...
    target = tmp_trn['log_leak'].values

    score_name = './model_data/model_2v4_featscores_%s.csv'%lagval
...
try:
    ...
    pass
fnames, _,  test = get_dataframes()

# Load leaks
leak_val=38
print '\nLoading train and test leaks...\n'
_, test_leak = load_leaks(leak_val)

# Rewrite train and test sets
right_idx = np.where(test_leak!=0)[0]
wrong_idx = np.where(test_leak==0)[0]
train = test.loc[right_idx]
test = test.loc[wrong_idx]
train['target'] = test_leak[right_idx]
train['log_leak'] = np.log1p(train['target'])

# Format target variable
target = train['target'].values
target_log = train['log_leak'].values

# Get scores
get_scores(train, fnames, leak_val)
